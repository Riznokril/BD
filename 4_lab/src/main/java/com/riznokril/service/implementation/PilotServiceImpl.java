package com.riznokril.service.implementation;

import com.riznokril.DAO.implementation.PilotDaoImpl;
import com.riznokril.entity.PilotEntity;
import com.riznokril.service.AbstractService;

import java.sql.SQLException;
import java.util.List;

public class PilotServiceImpl implements AbstractService<PilotEntity> {

    private final PilotDaoImpl dao = new PilotDaoImpl();

    @Override
    public List<PilotEntity> findAll() throws SQLException {
        return dao.findAll();
    }

    @Override
    public PilotEntity findById(Integer id) throws SQLException {
        return dao.findById(id);
    }

    @Override
    public void create(PilotEntity entity) throws SQLException {
        dao.create(entity);
    }

    @Override
    public void update(Integer id, PilotEntity entity) throws SQLException {
        dao.update(id, entity);
    }

    @Override
    public void delete(Integer id) throws SQLException {
        dao.delete(id);
    }
}